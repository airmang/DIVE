...
**Deferred to a follow-up (P2-38)** — **CLOSED in S-049 (2026-07-02)**: `preview.rs` hardcoded-Korean reuse strings lived in the `logs` vector (dev-facing panel), and the user-facing `message` field on that path was already English. S-049 introduced a `ReusedPreviewLogCode` enum (`running_server_detected` / `previously_started_reused`), replaced the two literals with codes, added `slide_in.preview.reused.*` ko/en keys, and localizes known codes in `PreviewTab` (unknown log lines pass through). See `design-s049.md`. The 15 higher-impact Theme-3 findings (all P1s + the headline roadmap leak + the parity lock) landed here in S-043.
