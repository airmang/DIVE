...
| [`014-unrestricted-project-kinds`](014-unrestricted-project-kinds/spec.md) | Implemented (S-072–S-075, live re-QA pending) | Round-5 umbrella from the 2026-08-24 team QA of the PRD flow. Records the owner principle as Constitution **VII Unrestricted Project Kinds** (1.2.0, ADR `adr-unrestricted-project-kinds.md`) and re-decides S-047 under it: removal of the S-049 exclusionary scaffolding and `plan_form_consistency` check (S-072), then — owner fresh look — the form taxonomy itself, leaving one stack confirmation (S-075); in-place PRD edits via `revise_*` / `remove_*` / `retire_acceptance_criterion` patch ops (S-073); Enter-to-send in the PRD-flow chats and a legible confirm gate (S-074). |
