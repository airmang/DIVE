...
| [`014-unrestricted-project-kinds`](014-unrestricted-project-kinds/spec.md) | Approved, in progress (S-072–S-074) | Round-5 umbrella from the 2026-08-24 team QA of the PRD flow. Records the owner principle as Constitution **VII Unrestricted Project Kinds** (1.2.0, ADR `adr-unrestricted-project-kinds.md`) and re-decides S-047 under it: multi-valued `ArchitectureDecision.forms` with per-form definitions, additive-only planner scaffolding, removal of the S-049 `plan_form_consistency` contradiction check (S-072); in-place PRD edits via `revise_*` / `remove_*` / `retire_acceptance_criterion` patch ops (S-073); Enter-to-send in the PRD-flow chats and a legible confirm gate (S-074). |
