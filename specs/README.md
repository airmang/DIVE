...
| [`013-public-release-readiness`](013-public-release-readiness/spec.md) | Approved, in progress (S-058–S-070) | Round-4 umbrella driven by the 2026-07-20 full-repo public-release code review (`docs/qa/public-release-code-review-2026-07-20.md`, 124 confirmed findings: 2 P0 / 35 P1 / 87 P2). Thirteen themes across four waves: public hygiene (copyright PDF removal, gitignore promotion, OFL notices, script/version truth), quick wins (Rust/FE dead code, loadTauri unification, silent-failure surfacing, tool security hardening, correctness bug pack), structure & performance (approval-pipeline unification, workspace_plan/supervisor/pi_sidecar/FE god-hook splits, performance pack), and the final history purge + public flip gate (S-070, strictly last). Gates the public repo flip synchronized with the 2026-08-14 presentation. |
| [`011-conference-demo-readiness`](011-conference-demo-readiness/spec.md) | Approved, in progress (S-050–S-057) | Round-3 umbrella driven by the 2026-07-10 conference-readiness QA (`docs/qa/conference-readiness-2026-07-10/report.md`, verdict NO-GO for an unscripted new-user live demo). Eight themes: plan-quality-gate re-tuning (P0-01), provider+model Pi executability preflight (P0-02), stall-timeout false errors (P1-01), PRD interview transparency + field provenance (P1-02/03), release-gate restoration (P1-04), Windows installed-app demo readiness (P1-05, demo-critical — the venue machine is Windows), demo UX polish (P2), and a GO-judgment exit gate (clean app-data E2E ×3, 0 false errors) plus a presentation fallback package for the 2026-08-14 conference demo. |
| [`014-unrestricted-project-kinds`](014-unrestricted-project-kinds/spec.md) | Implemented (S-072–S-074, live re-QA pending) | Round-5 umbrella from the 2026-08-24 team QA of the PRD flow. Records the owner principle as Constitution **VII Unrestricted Project Kinds** (1.2.0, ADR `adr-unrestricted-project-kinds.md`) and re-decides S-047 under it: multi-valued `ArchitectureDecision.forms` with per-form definitions, additive-only planner scaffolding, removal of the S-049 `plan_form_consistency` contradiction check (S-072); in-place PRD edits via `revise_*` / `remove_*` / `retire_acceptance_criterion` patch ops (S-073); Enter-to-send in the PRD-flow chats and a legible confirm gate (S-074). |
