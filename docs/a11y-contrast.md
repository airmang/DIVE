...
- 실제 컴퓨티드 스타일을 Playwright로 수집 → 클라이언트에서 `relativeLuminance()` 계산 → WCAG ratio 비교
- 다크 / 라이트 두 모드 모두 검사
- `prefers-reduced-motion: reduce` 에뮬레이션 상태에서 DOM에 `motion-reduce:*` 유틸리티가 실제로 적용되는지 확인
- 기존 23 스위트 + i18n/a11y/contrast = 26 스위트가 매 세션 자동 돌도록
