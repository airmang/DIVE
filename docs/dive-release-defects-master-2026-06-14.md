...
- [x] **D-31 Help `문서 보기`/`문제 신고` GitHub 404**
  - 변경: 공개 GitHub URL을 여는 경로를 제거하고 `?route=user-guide`/`?route=user-guide&doc=troubleshooting` 인앱 문서 화면을 추가. frontend menu event와 Rust native menu handler 모두 같은 user-guide route로 이동하며, `문제 신고`는 트러블슈팅 문서와 안내 toast를 표시.
