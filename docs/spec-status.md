...
| `change_step` / `retire_step` mutation behavior reserved in contracts but easy to misread as shipped | Updated in S-033: `retire_step` is now shipped with a visible remove path (`workspace_plan_remove_step`), the `plan_step_retired` event, export coverage, and tests. `change_step` remains future/contract-reserved unless separately implemented. The shipped 004/005 plan-mutation behaviors are `add_step` and `retire_step`. |
