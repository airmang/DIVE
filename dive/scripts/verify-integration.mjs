...
async function main() {
  ...
  const draftCount = await page.$$eval(
  ...
  const textareaNow = await page.$eval(
    '[data-testid="chat-input-textarea"]',
    (el) => el.disabled,
  );
  ...
  await page.waitForTimeout(150);
  const seededCount = await page.$$eval(
    '[data-testid="card-tile"]',
    (els) => els.length,
  );
  ...
  const verifiedCard = await page.$(
    '[data-testid="card-tile"][data-card-state="verified"]',
  );
  check("verified card found", !!verifiedCard);
  if (verifiedCard) {
    await verifiedCard.click();
    await page.waitForTimeout(350);
    const slideOpen = await page.$eval(
    ...
    const activeTab = await page.$eval(
      '[data-testid="slide-in-tab"][aria-selected="true"]',
  ...
  const decomposedCard = await page.$(
    '[data-testid="card-tile"][data-card-state="decomposed"]',
  );
  if (decomposedCard) {
    await decomposedCard.click();
    await page.waitForTimeout(200);
    const stillClosed = await page.$eval(
      '[data-testid="slide-in-panel"]',
      (el) => el.getAttribute("data-open"),
    );
    check(
