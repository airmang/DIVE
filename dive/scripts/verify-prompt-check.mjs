...
async function main() {
  ...
  const phase1 = await page.$eval(
    '[data-testid="prompt-check-dialog"]',
    (el) => el.getAttribute("data-phase"),
  );
  check("dialog in done phase", phase1 === "done", phase1);
  const issueCount = await page.$$eval(
  ...
  check(
    "textarea replaced with refined text",
    filled.includes("src/App.tsx"),
    filled,
  );
  const lastSent = await page.$eval(
    '[data-testid="last-sent"]',
    (el) => el.textContent ?? "",
  );
  check(
    "message not sent yet (apply-only)",
    lastSent.includes("(없음)"),
    lastSent.slice(0, 80),
  );
  ...
  await page.waitForSelector('[data-testid="prompt-check-dialog"]', { state: "detached" });
  const lastSent2 = await page.$eval(
    '[data-testid="last-sent"]',
    (el) => el.textContent ?? "",
  );
  check(
